...
export function ClientShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_0%,#ffe8fb_0%,#fff3fe_45%,#f8fafc_100%)]">
      <header className="sticky top-0 z-20 border-b border-fuchsia-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-600">espace client</p>
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-4 px-4 py-5 md:grid-cols-[240px_1fr]">
        <aside className="hidden h-fit rounded-2xl border border-fuchsia-100 bg-white/90 p-3 md:block">
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-20 border-t border-fuchsia-100 bg-white/95 backdrop-blur md:hidden">
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center rounded-lg px-2 py-2 text-[11px] ${
                  active ? 'bg-fuchsia-600 text-white' : 'text-slate-600'
                }`}
              >
